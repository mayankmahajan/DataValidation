...
class InstaMixin(object):
    def __init__(self, node):
        ...
        logger.info("Getting cubes database, as this is an insta node")
        self.instance = self.getInstanceId()
        self.resetCubesDatabase()
    ...
    def getCubesDatabase(self):
        self.pushMode("shell")
        if not self._cubes_db:
            self._cubes_db = self.sendCmd("cli -m enable -t 'show run' | grep 'insta instance %s cubes-database' | awk '{print $NF}'" % self.instance).strip()
        logger.debug("Cubes database: %s" % self._cubes_db)
        self.popMode()
        return self._cubes_db
